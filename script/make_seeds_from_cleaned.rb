...
#
# Generate events which look like this:
...
optparse = OptionParser.new do |opts|
  opts.banner = <<-EOS
Given a cleaned up wikipedia year page dump file, output a seeds file of events
This script should be run after seperate_sections.rb and clean_markup.rb have been run.
  
Usage: make_seeds_from_cleaned.rb -f FILE
  EOS
...
File.open(options[:filename]).each_line do |line|
  ...
  curr_date = d if d
  if curr_date && title
    puts "Event.create(title: %|#{title_prefix}#{title}|, date: %|#{curr_date}|, tags: %|#{tags}|)"
    count = count + 1
  end
end
